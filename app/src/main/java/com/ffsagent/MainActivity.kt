...
class MainActivity : Activity() {
    ...
    private fun buildUi() {
        ...
        // Start button
        val start = button("▶  START AGENT", true) { saveAndStart() }
        start.setTextSize(17f)
        content.addView(start, margin(0,0,0,12))
        ...
        val stop = button("STOP", false) { FfsAccessibilityService.requestStop() }
        stop.setTextColor(Color.WHITE); stop.setBackgroundResource(R.drawable.stop_button)
        liveHeader.addView(stop, LinearLayout.LayoutParams(dp(82),dp(40)))
